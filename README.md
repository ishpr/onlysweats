...
- **Sessions, not faces.** No people search, no member directory (partner discovery is opt-in on both sides, five first names at a time, and answers to the same freeze and verification rules as a public session), no "about me", no photos on session cards. Chat is scoped to a booking and closes a day after the session. Women-only sessions are enforced by the server.
