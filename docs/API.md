...
- Code: rules in `src/lib/pace/rules.ts`, SQL + transactions in
  ...
  types in `src/lib/pace/types.ts`, schema in `migrations/0002_pace.sql`.
...
- **Verification** (phone, selfie liveness, ID for women-only), report/block and
  the "felt like a date" report, fee disputes, the admin queue.
