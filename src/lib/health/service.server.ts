...
import type { HealthConnection, HealthCursor, HealthDataType, HealthRecord, HealthSyncInput, PrivateWorkout, WorkoutRecord } from "../../../shared/health.ts";
...
type ConnectionRow = {
  device_id: string; generation: string; types: HealthDataType[];
  since_at: Date | string; connected_at: Date | string; last_synced_at: Date | string | null;
};
const iso = (value: Date | string) => new Date(value).toISOString();
const canonicalTypes = (types: HealthDataType[]) => HEALTH_TYPES.filter((type) => types.includes(type));

async function connectionView(sql: Sql, userId: string, row: ConnectionRow): Promise<HealthConnection> {
  ...
  return {
    deviceId: row.device_id, generation: row.generation, types: row.types,
    sinceAt: iso(row.since_at), connectedAt: iso(row.connected_at),
    lastSyncedAt: row.last_synced_at === null ? null : iso(row.last_synced_at),
    cursors: Object.fromEntries(cursors.map(({ type, sequence, anchor }) => [type, { sequence, anchor }])),
  };
}
...
async function lockConnection(sql: Sql, userId: string): Promise<ConnectionRow | undefined> {
  ...
  const [connection] = await sql<ConnectionRow>`select * from health_connections where user_id = ${userId} for update`;
  return connection;
}
...
export async function connect(sql: Sql, userId: string, input: unknown, now = Date.now()): Promise<HealthConnection> {
  ...
  return sql.transaction(async (tx) => {
    ...
    if (current && current.device_id === parsed.deviceId && JSON.stringify(current.types) === JSON.stringify(types)) {
    ...
    const [row] = await tx<ConnectionRow>`
      insert into health_connections (user_id, device_id, generation, types, since_at, connected_at)
      values (${userId}, ${parsed.deviceId}, ${generation}, ${types}, ${sinceAt}, ${new Date(now).toISOString()})
      on conflict (user_id) do update set device_id = excluded.device_id, generation = excluded.generation,
        types = excluded.types, last_synced_at = null
      returning *`;
...
export async function sync(sql: Sql, userId: string, input: unknown, now = Date.now()): Promise<HealthConnection> {
  ...
  return sql.transaction(async (tx) => {
    ...
    if (!current.types.includes(page.type)) throw new HealthError(403, "This health type has not been selected.");
    const [cursor] = await tx<HealthCursor>`
      select sequence, anchor from health_cursors where user_id = ${userId} and type = ${page.type}`;
    if (!cursor || cursor.sequence !== page.expectedSequence) throw new HealthError(409, "Health cursor changed. Refresh before syncing.");
    if (page.records.some((record) => Date.parse(record.endAt) < Date.parse(iso(current.since_at)))) {
    ...
    const [updated] = await tx<ConnectionRow>`update health_connections set last_synced_at = ${new Date(now).toISOString()}
      where user_id = ${userId} returning *`;
    return connectionView(tx, userId, updated);
  });
}

type RecordRow = { external_id: string; record: WorkoutRecord; imported_at: Date | string; revision: number };
...
async function workoutView(sql: Sql, userId: string, row: RecordRow): Promise<PrivateWorkout> {
  ...
  const [heart] = await sql<{
    sample_count: number; min_bpm: number | null; max_bpm: number | null; sample_mean_bpm: number | null;
    first_sample_at: string | null; last_sample_at: string | null;
  }>`
  ...
  workout.heartRate = {
    availability: heart.sample_count ? "available" : "unavailable", sampleCount: heart.sample_count,
    minBpm: heart.min_bpm, maxBpm: heart.max_bpm, sampleMeanBpm: heart.sample_mean_bpm,
    firstSampleAt: heart.first_sample_at, lastSampleAt: heart.last_sample_at,
  };
  return workout;
}
...
function decodeCursor(cursor: string | undefined, kind: string): string[] | null {
  ...
  try {
    ...
    if (!Array.isArray(data) || data.length !== 3 || data[0] !== kind || data.some((v) => typeof v !== "string")) throw new Error();
    sourceId.parse(data[2]);
    if (kind === "workouts" && !/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d\.\d{3}Z$/.test(data[1])) throw new Error();
    if (kind === "export" && !HEALTH_TYPES.includes(data[1])) throw new Error();
    return data;
  } catch { throw new HealthError(400, "Invalid health page cursor."); }
}
const encodeCursor = (kind: string, position: string, id: string) => Buffer.from(JSON.stringify([kind, position, id])).toString("base64url");

export async function listWorkouts(sql: Sql, userId: string, page: PageArgs): Promise<{ workouts: PrivateWorkout[]; nextCursor: string | null }> {
  const cursor = decodeCursor(page.cursor, "workouts");
  const rows = await sql<RecordRow>`select external_id, record, imported_at, revision from health_records
  ...
  return {
    ...
    nextCursor: rows.length > page.limit && last ? encodeCursor("workouts", last.record.startAt, last.external_id) : null,
  };
}
...
export async function getWorkout(sql: Sql, userId: string, id: string): Promise<PrivateWorkout> {
  ...
  const [row] = await sql<RecordRow>`select external_id, record, imported_at, revision from health_records
...
export async function deleteWorkout(sql: Sql, userId: string, id: string): Promise<void> {
  ...
  await sql.transaction(async (tx) => {
    ...
    const rows = await tx`update health_records set revision = revision + case when record is not null then 1 else 0 end,
...
export async function exportRecords(sql: Sql, userId: string, page: PageArgs): Promise<{ records: HealthRecord[]; nextCursor: string | null }> {
  ...
  return {
    ...
    nextCursor: rows.length > page.limit && last ? encodeCursor("export", last.type, last.external_id) : null,
  };
}
