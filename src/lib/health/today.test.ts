...
const at = (hoursFromMidnight: number) => new Date(Date.parse(dayStart) + hoursFromMidnight * 3_600_000).toISOString();
...
const rec = (type: HealthDataType, from: number, to: number, rest: object, source = watch) =>
  ({ externalId: randomUUID(), source, startAt: at(from), endAt: at(to), type, ...rest }) as HealthRecord;

const ALL: HealthDataType[] = ["workout", "heart_rate", "resting_heart_rate", "heart_rate_variability", "sleep", "steps", "active_energy", "blood_glucose"];
async function member(records: HealthRecord[]) {
  ...
  let connection: HealthConnection = await health.connect(sql, id, { deviceId: randomUUID(), types: ALL }, now);
  for (const type of ALL) {
    const page = records.filter((record) => record.type === type);
    if (page.length === 0) continue;
    connection = await health.sync(sql, id, {
      deviceId: connection.deviceId, generation: connection.generation, type,
      expectedSequence: connection.cursors[type]?.sequence ?? 0, records: page, deletedIds: [], anchor: "anchor", hasMore: false,
    }, now);
  }
  return id;
}
...
describe("the day summary", () => {
  it("needs a connection and a plausible local midnight", async () => {
    ...
    await assert.rejects(today(sql, stranger, { dayStart }, now), (error: unknown) => error instanceof HealthError && error.status === 404);
    const id = await member([]);
    await assert.rejects(today(sql, id, { dayStart: "2026-09-10T05:00:00.000Z" }, now), (error: unknown) => error instanceof HealthError && error.status === 400);
  ...
  it("compares resting heart rate and HRV with the member's own week, and buckets today's heart rate", async () => {
    ...
    const id = await member([
      ...
      rec("workout", 7, 8, { activity: "run", durationSeconds: 3000, distanceMeters: 8046.72, activeEnergyKilocalories: 410 }),
      rec("workout", -40, -39, { activity: "strength", durationSeconds: 2400, distanceMeters: null, activeEnergyKilocalories: null }),
    ]);
    const { snapshot, trends } = await today(sql, id, { dayStart }, now);
    assert.equal(snapshot.restingHr, 63);
    assert.equal(snapshot.restingHrBase, 56);
    assert.equal(snapshot.weekWorkouts, 2);
    assert.deepEqual(snapshot.workouts.map((w) => [w.kind, w.minutes]), [["run", 50]]);
...
describe("reading the day", () => {
  const blank: DaySnapshot = { workouts: [], steps: null, sleepMin: null, sleepBaseMin: null, restingHr: null, restingHrBase: null, hrvMs: null, hrvBase: null, weekWorkouts: 0 };
  it("calls a short night easy and a full one ready, without diagnosing anything", () => {
    ...
    assert.equal(readToday({ ...blank, sleepMin: 6 * 60 + 30, sleepBaseMin: 8 * 60 }).lines[0], "You slept 6 h 30 min — less than you usually do.");
