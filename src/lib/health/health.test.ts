...
import type { HealthConnection, HealthDataType, HealthRecord, HealthSyncInput, QuantityRecord, WorkoutRecord } from "../../../shared/health.ts";
...
const workout = (patch: Partial<WorkoutRecord> = {}): WorkoutRecord => ({
  externalId: randomUUID(), source, startAt: "2026-09-20T10:00:00.000Z", endAt: "2026-09-20T11:00:00.000Z",
  type: "workout", activity: "run", durationSeconds: 3000, distanceMeters: 5000, activeEnergyKilocalories: null,
  ...patch,
});
const heart = (value = 140, patch: Partial<QuantityRecord> = {}): QuantityRecord => ({
  externalId: randomUUID(), source, startAt: "2026-09-20T10:20:00.000Z", endAt: "2026-09-20T10:20:00.000Z",
  type: "heart_rate", value, unit: "bpm", ...patch,
} as QuantityRecord);
const rejects = (promise: Promise<unknown>, status: number) => assert.rejects(promise, (error: unknown) => error instanceof HealthError && error.status === status);
...
function page(connection: HealthConnection, type: HealthDataType = "workout", patch: Partial<HealthSyncInput> = {}): HealthSyncInput {
  return {
    deviceId: connection.deviceId, generation: connection.generation, type,
    expectedSequence: connection.cursors[type]?.sequence ?? 0,
    records: [], deletedIds: [], anchor: "opaque-anchor-1", hasMore: false, ...patch,
  };
}
...
describe("private health connection and import", () => {
  it("requires explicit workout selection, deduplicates configuration order, and preserves cursors for an identical opt-in", async () => {
    ...
    const again = await health.connect(sql, id, { deviceId: connection.deviceId, types: ["heart_rate", "workout"] }, now + 1000);
    assert.deepEqual(again, synced);
    await assert.rejects(health.connect(sql, id, { deviceId: connection.deviceId, types: ["heart_rate"] }), z.ZodError);
    await assert.rejects(health.connect(sql, id, { deviceId: connection.deviceId, types: ["workout", "workout"] }), z.ZodError);
  });

  it("keeps one installation and rejects stale device/generation requests without advancing cursors", async () => {
    const { id, connection } = await member();
    const replacement = await health.connect(sql, id, { deviceId: randomUUID(), types: connection.types }, now + 86_400_000);
    assert.notEqual(replacement.generation, connection.generation);
    assert.equal(replacement.sinceAt, connection.sinceAt);
    await rejects(health.sync(sql, id, page(connection, "workout", { records: [workout()] }), now), 409);
    await rejects(health.sync(sql, id, page(replacement, "workout", { deviceId: connection.deviceId }), now), 409);
  ...
  it("commits records and anchors together; replayed acknowledgements conflict without duplicate data", async () => {
    ...
    const identical = await health.sync(sql, id, page(synced, "workout", { records: [record] }), now + 500);
    assert.equal((await health.getWorkout(sql, id, record.externalId)).revision, 1);
    const replayed = await health.sync(sql, id, page(identical, "workout", { records: [{ ...record, durationSeconds: 2900 }] }), now + 1000);
  ...
  it("isolates source IDs, reads, deletes, exports, and heart samples by signed-in member", async () => {
    ...
    assert.equal((await health.getWorkout(sql, a.id, record.externalId)).heartRate.availability, "unavailable");
  ...
  it("rejects a type the member did not opt into", async () => {
    ...
    await rejects(health.sync(sql, id, page(connection, "heart_rate", { records: [heart()] }), now), 403);
  ...
  it("stores authorized optional metrics with their source units and exports every selected type", async () => {
    ...
    const records: HealthRecord[] = [workout(), heart(),
      { ...base, externalId: randomUUID(), type: "resting_heart_rate", value: 62, unit: "bpm" },
      { ...base, externalId: randomUUID(), type: "heart_rate_variability", value: 45.5, unit: "ms" },
    ...
    await health.sync(sql, id, page(synced!, "heart_rate", { deletedIds: [records[1].externalId, records[1].externalId] }), now);
    assert.equal((await health.getWorkout(sql, id, records[0].externalId)).heartRate.availability, "unavailable");
    const [deleted] = await sql`select record, imported_at from health_records where user_id = ${id} and type = 'heart_rate'`;
  ...
  it("serializes duplicate pages and disconnect against in-flight synchronization", async () => {
    ...
    const outcomes = await Promise.allSettled([health.sync(sql, id, input, now), health.sync(sql, id, input, now)]);
    assert.equal(outcomes.filter((outcome) => outcome.status === "fulfilled").length, 1);
    const rejected = outcomes.find((outcome) => outcome.status === "rejected");
    assert.ok(rejected?.status === "rejected" && rejected.reason instanceof HealthError && rejected.reason.status === 409);
    const latest = await health.getConnection(sql, id);
    await Promise.all([health.sync(sql, id, page(latest!, "workout", { records: [workout()] }), now), health.disconnect(sql, id)]);
  ...
  it("permission removal purges measurements, rotates generation, and invalidates every in-flight page", async () => {
    ...
    const reduced = await health.connect(sql, id, { deviceId: connection.deviceId, types: ["workout"] }, now + 1000);
    ...
    await rejects(health.sync(sql, id, page(connection, "heart_rate", { expectedSequence: 1, records: [heart()] }), now), 409);
    assert.deepEqual((await health.exportRecords(sql, id, { limit: 200 })).records, [record]);
    assert.equal((await health.getWorkout(sql, id, record.externalId)).heartRate.availability, "unavailable");
  });

  it("user deletion scrubs the whole record and tombstones block reimport, including uppercase UUIDs", async () => {
    const { id, connection } = await member();
    const record = workout();
    const synced = await health.sync(sql, id, page(connection, "workout", { records: [record] }), now);
    await health.deleteWorkout(sql, id, record.externalId);
    await health.deleteWorkout(sql, id, record.externalId);
    const [tombstone] = await sql`select * from health_records where user_id = ${id}`;
    assert.deepEqual(tombstone, { user_id: id, type: "workout", external_id: record.externalId, record: null, imported_at: null, revision: 2 });
    await health.sync(sql, id, page(synced, "workout", { records: [{ ...record, externalId: record.externalId.toUpperCase() }] }), now);
  ...
  it("HealthKit deletions win within a page and before a later import; replacement source UUIDs still work", async () => {
    ...
    const synced = await health.sync(sql, id, page(connection, "workout", { records: [first], deletedIds: [first.externalId, future.externalId] }), now);
    await health.sync(sql, id, page(synced, "workout", { records: [first, future, workout()] }), now);
  ...
  it("disconnect purges data, tombstones and cursors; reconnect creates a new authorization generation", async () => {
    const { id, connection } = await member();
    const record = workout();
    await health.sync(sql, id, page(connection, "workout", { records: [record], deletedIds: [randomUUID()] }), now);
    ...
    await rejects(health.sync(sql, id, page(connection, "workout", { expectedSequence: 1 }), now), 409);
    const reconnected = await health.connect(sql, id, { deviceId: connection.deviceId, types: connection.types }, now);
  ...
  it("auth identity deletion cascades all health data without deleting another member", async () => {
    const a = await member();
    const b = await member();
    await health.sync(sql, a.id, page(a.connection, "workout", { records: [workout()], deletedIds: [randomUUID()] }), now);
    await sql`delete from "user" where id = ${a.id}`;
    for (const table of ["health_connections", "health_cursors", "health_records"]) {
      assert.equal((await sql.query(`select * from ${table} where user_id = $1`, [a.id])).length, 0);
    }
  ...
  it("rejects an invalid batch without storing any records, deletions or anchor", async () => {
    const { id, connection } = await member();
    const valid = workout();
    await assert.rejects(health.sync(sql, id, page(connection, "workout", {
      records: [valid, { ...workout(), distanceMeters: -1 }], deletedIds: [randomUUID()],
    }), now), z.ZodError);
    assert.equal((await sql`select * from health_records where user_id = ${id}`).length, 0);
    assert.deepEqual((await health.getConnection(sql, id))?.cursors.workout, { sequence: 0, anchor: null });
    await rejects(health.sync(sql, id, page(connection, "workout", {
      records: [valid, workout({ startAt: "2026-09-22T10:00:00.000Z", endAt: "2026-09-22T11:00:00.000Z" })],
    }), now), 400);
    assert.equal((await sql`select * from health_records where user_id = ${id}`).length, 0);
  });
  ...
  it("rolls back preceding writes and keeps the old anchor when a database write fails", async () => {
    ...
    await sql.query(`alter table health_records add constraint health_test_failure check (external_id <> '${failed.externalId}'::uuid)`);
    try {
      await assert.rejects(health.sync(sql, id, page(connection, "workout", { records: [workout(), failed] }), now));
      assert.equal((await sql`select * from health_records where user_id = ${id}`).length, 0);
      assert.deepEqual((await health.getConnection(sql, id))?.cursors.workout, { sequence: 0, anchor: null });
    } finally { await sql`alter table health_records drop constraint health_test_failure`; }
  });
  ...
  it("allows a sleep interval overlapping the fixed initial boundary but rejects older samples", async () => {
    ...
    const sleep: HealthRecord = {
      externalId: randomUUID(), source, type: "sleep", stage: "asleep_unspecified",
      startAt: "2026-08-22T10:00:00.000Z", endAt: "2026-08-22T13:00:00.000Z",
    };
    const synced = await health.sync(sql, id, page(connection, "sleep", { records: [sleep] }), now);
    await rejects(health.sync(sql, id, page(synced, "sleep", { records: [{ ...sleep, externalId: randomUUID(), endAt: "2026-08-22T11:00:00.000Z" }] }), now), 400);
  });
  ...
  it("uses stable pagination and rejects malformed or cross-kind page cursors", async () => {
    ...
    const remainder = await health.exportRecords(sql, id, { limit: 2, cursor: exported.nextCursor! });
  ...
  it("summarizes source-reported duration and only same-source heart rate inside this workout", async () => {
    ...
    const record = workout();
    await health.sync(sql, id, page(connection, "workout", { records: [record] }), now);
    await health.sync(sql, id, page(connection, "heart_rate", { records: [
      heart(120), heart(160, { startAt: "2026-09-20T10:30:00.000Z", endAt: "2026-09-20T10:30:00.000Z" }),
    ...
    assert.deepEqual(result.heartRate, { availability: "available", sampleCount: 2, minBpm: 120, maxBpm: 160,
      sampleMeanBpm: 140, firstSampleAt: "2026-09-20T10:20:00.000Z", lastSampleAt: "2026-09-20T10:30:00.000Z" });
  });
});
...
describe("health validation and deterministic summaries", () => {
  it("rejects mismatched units/types, non-finite/negative quantities, bad time ranges and unknown fields", () => {
    for (const invalid of [
      { ...heart(), unit: "ms" }, { ...heart(), value: Infinity }, { ...heart(), value: NaN },
      { ...heart(), value: -1 }, { ...heart(), value: 0 }, { ...heart(), privateNote: "not permitted" },
      { ...heart(), startAt: "2026-09-20T11:00:00.000Z" },
      { ...workout(), durationSeconds: 3602 }, { ...workout(), durationSeconds: -1 },
      { ...workout(), distanceMeters: Infinity }, { ...heart(), type: "steps", unit: "count", value: 1.5 },
    ]) assert.equal(healthRecord.safeParse(invalid).success, false);
  });

  it("bounds pages, anchors and duplicate source IDs before import", () => {
    const connection = { deviceId: randomUUID(), generation: randomUUID(), cursors: {} } as HealthConnection;
    const record = workout();
    for (const invalid of [
  ...
  it("keeps missing data explicit and calculates pace only from eligible activity and measured distance", () => {
    ...
    for (const patch of [{ activity: "ride" as const }, { distanceMeters: null }, { distanceMeters: 0 }, { durationSeconds: 0 }]) {
      const unavailable = summarizeWorkout({ ...record, ...patch }, new Date(now).toISOString(), []);
...
describe("health HTTP boundary", () => {
  ...
  it("accepts JSON and rejects oversized declared or streamed bodies", async () => {
    const request = (body: string, headers?: HeadersInit) => new Request("https://example.test/health/sync", { method: "POST", body, headers });
    assert.deepEqual(await readHealthBody(request('{"records":[]}')), { records: [] });
    await rejects(readHealthBody(request("{}", { "content-length": String(MAX_HEALTH_BODY_BYTES + 1) })), 413);
