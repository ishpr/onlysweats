...
import { z } from "zod";
import { FITNESS_PILOT_NOTICE_VERSION } from "../../../shared/fitness-outcomes.ts";
...
import {
  ...
  type ExerciseDraft,
  type FitnessConsent,
  type FitnessAssessmentMetadata,
  type FitnessExportPage,
  type FitnessExportRecord,
  type StrengthLog,
...
import {
  ...
  emptyDraft,
  exerciseQuestions,
  EXERCISE_QUESTION_VERSION,
  numericCandidates,
  statedUnits,
  workoutQuestions,
  WORKOUT_QUESTION_VERSION,
} from "./questions.ts";
import {
  inferenceAvailable,
  productionProvider,
  ...
  type ProviderResult,
} from "./typesafe.server.ts";

import {
  attachPilotDraft,
  finishSaveMeasurement,
  loggingSessionLinks,
  preparePilotDraft,
  prepareSaveMeasurement,
  readLoggingOutcome,
  readPilotConsent,
  refreshSavedMeasurement,
  revokePilotMeasurements,
} from "./outcomes.server.ts";
...
function consentView(row?: ConsentRow): FitnessConsent {
  return {
    enabled: Boolean(row?.enabled && row.notice_version === FITNESS_AI_NOTICE_VERSION),
    generation: row?.generation ?? null,
    updatedAt: row ? iso(row.updated_at) : null,
    noticeVersion: FITNESS_AI_NOTICE_VERSION,
    providerAvailable: inferenceAvailable(),
  };
}
export async function getConsent(sql: Sql, userId: string): Promise<FitnessConsent> {
  return sql.transaction(async (tx) => {
    await lockOwner(tx, userId);
    return consentView(await consentRow(tx, userId));
  });
}
export async function setConsent(
  ...
  input: unknown,
  now = Date.now(),
): Promise<FitnessConsent> {
  const { enabled } = consentInput.parse(input);
  return sql.transaction(async (tx) => {
    await lockOwner(tx, userId);
    const previous = await consentRow(tx, userId);
    if (!enabled) await revokePilotMeasurements(tx, userId, now);
    if (previous?.enabled === enabled && previous.notice_version === FITNESS_AI_NOTICE_VERSION)
      return consentView(previous);
    const [row] =
      await tx<ConsentRow>`insert into fitness_consents (user_id, enabled, generation, notice_version, updated_at)
...
function requireConsent(row: ConsentRow | undefined): ConsentRow {
  if (!row?.enabled || row.notice_version !== FITNESS_AI_NOTICE_VERSION)
    throw new FitnessError(
      403,
      "Enable AI assistance separately before sending a note to TypeSafe.",
    );
  return row;
}
async function reserveInference(sql: Sql, userId: string, now: number): Promise<ConsentRow> {
  ...
  if (
    consent.last_requested_at !== null &&
    now - new Date(consent.last_requested_at).getTime() < 3000
  )
    throw new FitnessError(429, "Please wait a few seconds before requesting another suggestion.");
  const day = new Date(now).toISOString().slice(0, 10);
  const count =
    consent.request_day && iso(consent.request_day).slice(0, 10) === day
      ? consent.request_count
      : 0;
  if (count >= 60)
    throw new FitnessError(
      429,
      "Today's AI suggestion limit is reached. Manual logging is still available.",
    );
...
function logView(row: LogRow, measurementSessionId?: string): StrengthLog {
  const totalRepetitions = row.data.sets.reduce((sum, set) => sum + set.reps, 0);
  const totalVolumeKg = row.data.sets.some(
    (set) => set.unit === "bodyweight" || set.weight === null,
  )
    ? null
    : Math.round(
        row.data.sets.reduce(
          (sum, set) => sum + set.reps * (set.weight ?? 0) * (set.unit === "lb" ? 0.45359237 : 1),
          0,
        ) * 1000,
      ) / 1000;
  return {
...
function checkLogTime(input: StrengthLogInput, now: number) {
  ...
  if (started > now + 300_000 || started < Date.UTC(2000, 0, 1))
    throw new FitnessError(400, "Choose a valid past workout time.");
}
export async function createStrengthLog(
  ...
  const { measurement, ...parsed } = createStrengthLogInput.parse(input);
  checkLogTime(parsed, now);
  return sql.transaction(async (tx) => {
    await lockOwner(tx, userId);
    const prepared = await prepareSaveMeasurement(tx, userId, measurement, parsed, now);
    if (prepared?.existingLogId) {
      const [existing] =
        await tx<LogRow>`select * from fitness_strength_logs where user_id = ${userId} and id = ${prepared.existingLogId}`;
      return logView(existing, prepared.row.id);
    }
    const [row] =
...
export async function updateStrengthLog(
  ...
  return sql.transaction(async (tx) => {
    await lockOwner(tx, userId);
    const [current] =
      await tx<LogRow>`select * from fitness_strength_logs where user_id = ${userId} and id = ${logId}`;
    if (!current) throw new FitnessError(404, "Exercise log not found.");
    if (current.revision !== expectedRevision)
      throw new FitnessError(409, "This log changed. Reload it before editing.");
    const [row] =
...
function decodeCursor(input: string | undefined, kind: string): [string, string] | null {
  ...
  try {
    const parsed = z
      .tuple([z.literal(kind), z.string().max(50), fitnessId])
      .parse(JSON.parse(Buffer.from(input, "base64url").toString("utf8")));
    return [parsed[1], parsed[2]];
  } catch {
    throw new FitnessError(400, "Invalid page cursor.");
  }
}
...
export async function listStrengthLogs(
  sql: Sql,
  userId: string,
  input: FitnessPageArgs,
): Promise<{ logs: StrengthLog[]; nextCursor: string | null }> {
  ...
  const links = await loggingSessionLinks(
    sql,
    userId,
    visible.map((row) => row.id),
  );
  return {
    logs: visible.map((row) => logView(row, links.get(row.id))),
    nextCursor:
      rows.length > page.limit
        ? encodeCursor("logs", visible.at(-1)!.data.startedAt, visible.at(-1)!.id)
        : null,
  };
}
async function ownedWorkout(sql: Sql, userId: string, id: string) {
  try {
    return await getWorkout(sql, userId, id);
  } catch (error) {
    if (error instanceof HealthError) throw new FitnessError(error.status, error.message);
    throw error;
  }
}
async function readCorrection(
  ...
  userId: string,
  workoutId: string,
): Promise<WorkoutCorrection | null> {
  const [row] =
    await sql<CorrectionRow>`select * from fitness_workout_corrections where user_id = ${userId} and workout_id = ${workoutId}`;
...
export async function getCorrection(
  ...
  return sql.transaction(async (tx) => {
    ...
    return readCorrection(tx, userId, workoutId);
  });
}
export async function saveCorrection(
  ...
  return sql.transaction(async (tx) => {
    ...
    if ((previous?.revision ?? 0) !== expectedRevision)
      throw new FitnessError(409, "This correction changed. Reload it before editing.");
    const [row] =
      await tx<CorrectionRow>`insert into fitness_workout_corrections (user_id, workout_id, data, updated_at)
...
function metadata(
  ...
  now: number,
): FitnessAssessmentMetadata {
  return { ...result, snapshotRevision, questionVersion, assessedAt: new Date(now).toISOString() };
}
export async function draftExercise(
  sql: Sql,
  userId: string,
  input: unknown,
  options: InferenceOptions = {},
): Promise<ExerciseDraft> {
  const { note, loggingSessionId } = noteInput.parse(input);
  ...
  const prepared = await sql.transaction(async (tx) => {
    ...
    const measurement = await preparePilotDraft(tx, userId, loggingSessionId, now);
    if (candidates.length > 32 || (!options.provider && !inferenceAvailable()))
      return { consent: current, measurement };
    return { consent: await reserveInference(tx, userId, now), measurement };
  });
  const snapshotRevision = fingerprint({
    userId,
    generation: prepared.consent.generation,
    notice: FITNESS_AI_NOTICE_VERSION,
    note,
  });
  const fallback =
    candidates.length > 32
      ? "insufficient_data"
      : !options.provider && !inferenceAvailable()
  ...
  const result = fallback
    ...
    : await (options.provider ?? productionProvider)(
        { note, candidates },
        exerciseQuestions(candidates, units),
      );
  return sql.transaction(async (tx) => {
    await lockOwner(tx, userId);
    const current = requireConsent(await consentRow(tx, userId));
    if (current.generation !== prepared.consent.generation)
      throw new FitnessError(
        409,
        "AI permission changed while preparing this draft. Please try again.",
      );
    const finishedAt = options.now ?? Date.now();
    const draft: ExerciseDraft = result
      ? {
          ...draftFromAnswers(result.answers, candidates, units),
          metadata: metadata(result, snapshotRevision, EXERCISE_QUESTION_VERSION, finishedAt),
        }
      : emptyDraft(fallback ?? "provider_unavailable");
    return attachPilotDraft(tx, userId, prepared.measurement, draft, finishedAt);
  });
}
async function workoutSnapshot(
  sql: Sql,
  userId: string,
  workoutId: string,
  request: z.infer<typeof assessmentInput>,
) {
  const consent = requireConsent(await consentRow(sql, userId));
  ...
  if (!connection)
    throw new FitnessError(
      409,
      "Reconnect Apple Health before requesting a workout interpretation.",
    );
  ...
  const snapshotRevision = fingerprint({
    ...
    correction,
    request,
    questionVersion: WORKOUT_QUESTION_VERSION,
  });
  ...
  const state = {
    ...
    note: request.note,
    workout: {
      sourceActivity: workout.record.activity,
      memberActivity: correction?.activity ?? null,
      durationSeconds: workout.record.durationSeconds,
      distanceMeters: workout.record.distanceMeters,
    },
  };
  return { snapshotRevision, state, endAt: workout.record.endAt };
}
export async function assessWorkout(
  sql: Sql,
  userId: string,
  id: string,
  input: unknown,
  options: InferenceOptions = {},
): Promise<WorkoutAssessment> {
  const workoutId = fitnessId.parse(id);
  ...
  const prepared = await sql.transaction(async (tx) => {
    ...
    const insufficient =
      request.note.length < 8 ||
      request.goal.length < 3 ||
      now - new Date(connection!.lastSyncedAt ?? 0).getTime() > 7 * 86_400_000 ||
      new Date(snapshot.endAt).getTime() > now + 300_000;
    if (!insufficient && (options.provider || inferenceAvailable()))
      await reserveInference(tx, userId, now);
  ...
  if (!options.provider && !inferenceAvailable())
  ...
  return sql.transaction(async (tx) => {
    ...
    if (latest.snapshotRevision !== prepared.snapshotRevision)
      throw new FitnessError(
        409,
        "This workout or its permissions changed. Request a fresh interpretation.",
      );
    if (!result) return { ...base, status: "provider_unavailable" };
    const picked = acceptedChoice(result.answers.interpretation);
    const interpretation =
      picked === "goal_aligned" || picked === "intentional_change" ? picked : "unclear";
    const assessment: WorkoutAssessment = {
      ...base,
      status: interpretation === "unclear" ? "insufficient_data" : "available",
      interpretation,
      metadata: metadata(
        result,
...
async function currentAssessment(
  ...
  workoutId: string,
  row: AssessmentRow,
): Promise<WorkoutAssessment | null> {
  try {
    ...
    if (!(error instanceof FitnessError)) throw error;
  }
...
export async function getWorkoutAssessment(
  ...
  return sql.transaction(async (tx) => {
    ...
    await ownedWorkout(tx, userId, workoutId);
    const [row] =
      await tx<AssessmentRow>`select request, result from fitness_workout_assessments where user_id = ${userId} and workout_id = ${workoutId}`;
    if (!row) return null;
    const current = await currentAssessment(tx, userId, workoutId, row);
    return current ? { ...current, request: row.request } : null;
  });
}
export async function exportFitness(
  sql: Sql,
  userId: string,
  input: FitnessPageArgs,
): Promise<FitnessExportPage> {
  const page = fitnessPageInput.parse(input);
  ...
  return sql.transaction(async (tx) => {
    ...
    const rows = await tx<{ kind: string; id: string }>`select kind, id from (
      select 'pilot_outcome' as kind, s.id from fitness_logging_sessions s join fitness_pilot_consents c on c.user_id = s.user_id
        where s.user_id = ${userId} and c.enabled and c.notice_version = ${FITNESS_PILOT_NOTICE_VERSION} and s.consent_generation = c.generation and s.started_at >= ${new Date(exportTime - 30 * 86400_000)}
    ...
    for (const row of visible) {
      if (row.kind === "pilot_outcome") {
        records.push({
          kind: "pilot_outcome",
          value: await readLoggingOutcome(tx, userId, row.id, exportTime),
        });
      } else if (row.kind === "strength_log") {
        const [log] =
          await tx<LogRow>`select * from fitness_strength_logs where user_id = ${userId} and id = ${row.id}`;
        ...
        if (value)
          records.push({
            kind: "workout_assessment",
            value: { ...value, request: assessment.request },
          });
      }
    }
    const last = visible.at(-1);
    return {
      consent: consentView(await consentRow(tx, userId)),
      pilotConsent: await readPilotConsent(tx, userId),
      records,
      nextCursor:
        rows.length > page.limit && last
          ? encodeCursor("fitness-export", last.kind, last.id)
          : null,
    };
  });
}
