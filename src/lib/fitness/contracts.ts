...
export const noteInput = z.strictObject({
  ...
  loggingSessionId: fitnessId.optional(),
});
export const saveMeasurementInput = z.strictObject({
  sessionId: fitnessId,
  draftId: fitnessId.optional(),
});
export const assessmentInput = z.strictObject({
  goal: z.string().trim().min(1).max(300),
  note: z.string().trim().max(1000),
});
export const strengthSet = z
  .strictObject({
...
export const strengthLogInput = z
  .strictObject({
    ...
    note: z.string().trim().max(1000),
    sets: z.array(strengthSet).min(1).max(50),
  })
  .refine((log) => log.exerciseId !== "other" || log.note.length > 0, {
    message: "Name the other exercise in your note.",
  });
export const createStrengthLogInput = strengthLogInput.safeExtend({
  measurement: saveMeasurementInput.optional(),
});
export const updateStrengthLogInput = strengthLogInput.safeExtend({
