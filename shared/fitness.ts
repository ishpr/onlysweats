...
export const EXERCISE_CATALOGUE = [
  ...
  { id: "lunge", name: "Lunge" },
  { id: "biceps_curl", name: "Biceps curl" },
  { id: "triceps_extension", name: "Triceps extension" },
...
export type StrengthLogSaveInput = StrengthLogInput & { measurement?: FitnessSaveMeasurement };
export type StrengthLog = StrengthLogInput & {
  id: string;
  revision: number;
  createdAt: string;
  updatedAt: string;
  /** Optional, current consented feedback receipt; never part of the workout itself. */
...
export type ExerciseDraft = {
  measurement?: FitnessDraftMeasurement;
...
export type FitnessExportRecord =
  ...
  | {
      ...
      value: WorkoutAssessment & { request: { goal: string; note: string } };
    };
export type FitnessExportPage = {
  consent: FitnessConsent;
  pilotConsent: FitnessPilotConsent;
  records: FitnessExportRecord[];
