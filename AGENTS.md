...
Follow [the Apple Health guide](docs/APPLE-HEALTH.md) for the import layer, [the fitness integration guide](docs/JEV-FITNESS.md) for the implemented server adapter and editable drafts, and [the A2A guide](docs/A2A.md) for the negotiation and booking boundary. Jev requires server configuration and an authorized member grant. Current product Terms acceptance initializes bounded coaching and Jev grants for members without a previous choice; it never restores existing opt-outs. Keep model selection server-side and coaching built into the product, without a separate AI activation or model-picker screen. OS HealthKit, shared planning, payments and pilot analytics retain their own boundaries. See `shared/app-terms.ts` and `docs/INTELLIGENCE.md`. Live synthetic evaluations are recorded under `docs/evaluations`; representative member acceptance remains pending. Never describe fixture results as general accuracy or infer production readiness from typed outputs.
